...
function Thumb({ article, className = '' }) {
  // An article with no thumbnail gets a tinted placeholder at the right aspect
  // ratio rather than a broken image. Keeping the box the same size either way
  // means adding artwork later does not reflow the list.
  if (!article.thumb) {
    return (
      <div
        className={`article-thumb is-empty ${className}`}
        style={{ '--accent': article.accent ?? 'var(--accent)' }}
        aria-hidden
      >
        <span>1500 × 600 thumbnail</span>
      </div>
    );
  }
      ...
      <section className="hero">
        ...
        <p>
          {IS_MAINNET
            ...
            : 'CEOs.fun is not live yet. Everything on the site is populated with example data, and anything below can change before launch.'}
        </p>
      </section>
